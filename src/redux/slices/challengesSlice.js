    ...
    async (_,{rejectWithValue})=>
        {
            try
            {
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/weekly-challenges/weekly`,
                {
                    method:"GET",
                    credentials:"include",
                    headers:
...
export const fetchpastchallenges = createAsyncThunk(
    ...
    async ({skip = 0, limit = 10},{rejectWithValue})=>
    {
        try
        {
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/weekly-challenges/past?skip=${skip}&limit=${limit}`,
                {
                    method:"GET",
                    credentials:"include"
...
export const fetchnormalchallenges = createAsyncThunk(
    ...
    async(_,{rejectWithValue}) =>{
        ...
        try{
            ...
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/weekly-challenges/normal`,
                {
                    method:"GET",
                    credentials:"include",
                    headers:
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/api/weekly-challenges/calendar`,
        {
          method: "GET",
          credentials: "include"
        }
